...
CMyCamera::CMyCamera(MV_CC_DEVICE_INFO*  device, QHalconWindow* win)
{
    m_hDevHandle        = NULL;
    m_device = device;
    hwin = win;
...
void CMyCamera::sendFrameReceivedSignal1(bool state, unsigned char *data)
{
    emit frameReceivedSignal1(state, data);
}

void CMyCamera::ImageCallBackEx1(unsigned char *pData, MV_FRAME_OUT_INFO_EX *pFrameInfo, void *pUser)
{
    CMyCamera* user = (CMyCamera*)pUser;
    if (pFrameInfo)
    {
        try
        {
            user->sendFrameReceivedSignal1(true, pData);
            //emit frameReceivedSignal1(true);
        }
        catch(HException& e)
        {
            HTuple  Exception;
            e.ToHTuple(&Exception);
            QString error = QString("Cam1 GenImage Exception")+QString(Exception.ToString().Text());
            qDebug()<<error;
    ...
    {
        user->sendFrameReceivedSignal1(false, pData);
        //emit frameReceivedSignal1(false);
        qDebug()<<"Cam1 grab failed";
    }
}

void CMyCamera::sendFrameReceivedSignal2(bool state, unsigned char *data)
{
    emit frameReceivedSignal2(state, data);
}

void CMyCamera::ImageCallBackEx2(unsigned char *pData, MV_FRAME_OUT_INFO_EX *pFrameInfo, void *pUser)
{
    CMyCamera* user = (CMyCamera*)pUser;
    if (pFrameInfo)
    {
        try
        {
            user->sendFrameReceivedSignal2(true, pData);
            //emit frameReceivedSignal1(true);
        }
        catch(HException& e)
        {
            HTuple  Exception;
            e.ToHTuple(&Exception);
            QString error = QString("Cam2 GenImage Exception")+QString(Exception.ToString().Text());
            qDebug()<<error;
            //emit updateLog(error);
        }
    }
    ...
    {
        user->sendFrameReceivedSignal2(false, pData);
        //emit frameReceivedSignal1(false);
        qDebug()<<"Cam2 grab failed";
    }
}

void CMyCamera::sendFrameReceivedSignal3(bool state, unsigned char *data)
{
    emit frameReceivedSignal3(state, data);
}

void CMyCamera::ImageCallBackEx3(unsigned char *pData, MV_FRAME_OUT_INFO_EX *pFrameInfo, void *pUser)
{
    CMyCamera* user = (CMyCamera*)pUser;
    if (pFrameInfo)
    {
        try
        {
            user->sendFrameReceivedSignal3(true, pData);
            //emit frameReceivedSignal1(true);
        }
        catch(HException& e)
        {
            HTuple  Exception;
            e.ToHTuple(&Exception);
            QString error = QString("Cam3 GenImage Exception")+QString(Exception.ToString().Text());
            qDebug()<<error;
            //emit updateLog(error);
        }
    }
    ...
    {
        user->sendFrameReceivedSignal3(false, pData);
        //emit frameReceivedSignal1(false);
        qDebug()<<"Cam3 grab failed";
    }
}


void CMyCamera::sendFrameReceivedSignal4(bool state, unsigned char *data)
{
    emit frameReceivedSignal4(state, data);
}

void CMyCamera::ImageCallBackEx4(unsigned char *pData, MV_FRAME_OUT_INFO_EX *pFrameInfo, void *pUser)
{
    CMyCamera* user = (CMyCamera*)pUser;
    if (pFrameInfo)
    {
        try
        {
            user->sendFrameReceivedSignal4(true, pData);
            //emit frameReceivedSignal1(true);
        }
        catch(HException& e)
        {
            HTuple  Exception;
            e.ToHTuple(&Exception);
            QString error = QString("Cam4 GenImage Exception")+QString(Exception.ToString().Text());
            qDebug()<<error;
            //emit updateLog(error);
        }
    }
    ...
    {
        user->sendFrameReceivedSignal4(false, pData);
        //emit frameReceivedSignal1(false);
        qDebug()<<"Cam4 grab failed";
    }
}


void CMyCamera::sendFrameReceivedSignal5(bool state, unsigned char *data)
{
    emit frameReceivedSignal5(state, data);
}

void CMyCamera::ImageCallBackEx5(unsigned char *pData, MV_FRAME_OUT_INFO_EX *pFrameInfo, void *pUser)
{
    CMyCamera* user = (CMyCamera*)pUser;
    if (pFrameInfo)
    {
        try
        {
            user->sendFrameReceivedSignal5(true, pData);
            //emit frameReceivedSignal1(true);
        }
        catch(HException& e)
        {
            HTuple  Exception;
            e.ToHTuple(&Exception);
            QString error = QString("Cam5 GenImage Exception")+QString(Exception.ToString().Text());
            qDebug()<<error;
            //emit updateLog(error);
        }
    }
    ...
    {
        user->sendFrameReceivedSignal5(false, pData);
        //emit frameReceivedSignal1(false);
        qDebug()<<"Cam5 grab failed";
    }
}
...
int     CMyCamera::Open()
{
    ...
    {
        nRet  = MV_CC_CreateHandle(&m_hDevHandle, m_device);
        if (MV_OK != nRet)
        {
            return nRet;
        }
